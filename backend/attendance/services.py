...
def calculate_daily_summary(employee: Employee, date: datetime.date) -> AttendanceSummary:
    ...
    # Punches exist: first punch is check-in, last is check-out (if count > 1)
    check_in = logs.first().timestamp
    if logs.count() > 1:
        check_out = logs.last().timestamp
    ...
    if working_hours == 0.00:
        ...
        status = 'HALF_DAY'
        remarks = f"Half day: Worked {working_hours} hours."
    else:
        status = 'ABSENT'
        remarks = f"Absent: Worked only {working_hours} hours."
