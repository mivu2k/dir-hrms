    ...
    <div class="flex items-center justify-between bg-slate-900/20 p-4 border border-slate-900 rounded-xl">
      ...
      <div class="flex items-center space-x-3">
        <button 
          ...
          @click="clearMockData"
          class="border border-rose-500/30 hover:bg-rose-500/10 text-rose-400 font-medium px-4 py-2 rounded-lg text-xs transition-colors flex items-center space-x-2 cursor-pointer"
        >
...
const clearMockData = async () => {
  if (!confirm('Are you sure you want to delete all simulated logs and summaries? This will wipe the mock charts and logs.')) return;
  try {
    const res = await axios.post('/devices/clear-mock-data');
